...
class ResNet1DEncoder(nn.Module):
    ...
    def __init__(self, in_channels=1, base_filters=32, encoder_dim=64,
        ...
        self.fc = nn.Linear(base_filters * 8, encoder_dim)
    ...
    def forward(self, x):
        # (B, 1, 1024)
        out = F.relu(self.bn1(self.conv1(x)))   # (B, 32, 512)
        out = self.maxpool(out)                  # (B, 32, 256)
        out = self.layer1(out)                    # (B, 32, 256)
        out = self.layer2(out)                    # (B, 64, 128)
        out = self.layer3(out)                    # (B, 128, 64)
        out = self.layer4(out)                    # (B, 256, 32)
        out = self.avgpool(out).squeeze(-1)       # (B, 256)
